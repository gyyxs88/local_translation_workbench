...
def test_cli_project_list_exposes_user_facing_project_metadata(
    ...
    assert exit_code == 0
    assert projects[chaptering_project.id]["title"] == "星灯旅人的未竟书"
    assert projects[chaptering_project.id]["source_path"] == "D:\\Novel\\星灯旅人的未竟书.txt"
...
def test_cli_project_list_flags_duplicate_source_paths(
    ...
    assert projects[first.id]["duplicate_group_key"] == "d:/novel/青灯小先生.txt"
    ...
    assert projects[unique.id]["duplicate_group_key"] == "d:/novel/山河医者.txt"
    ...
    payload = route_action(
        {
            ...
            "model_profile_id": "gpt_5_4_profile",
        }
    )
