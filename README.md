...
- 截至 `2026-04-16`，已验证的完整回归基线为：`218 passed`
...
- 抽取 prompt 要求模型直接返回 JSON，当前收口字段对齐生产侧常见口径：`source_term / translated_term / category / note / term_group_key / relation_role`。
- 本地正式术语表当前保存为 `source_term / target_term / category / note / term_group_key / relation_role`；`gender` 暂未单独建模。
...
- 译文版本里的 `glossary_snapshot_id` 现在基于当前有效术语表实时计算，不再写死占位值。
