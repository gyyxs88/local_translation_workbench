...
- 将 `glossary_multi_llm_v1` 从顺序双 extractor 升级为真实并发执行。
- 将 `translation_multi_llm_v1` 从顺序多轮链路升级为可控并发执行。
...
- 聚合阶段耗时、失败类型、fallback 命中、重试结果等关键信息。
- 优化 resume/rerun 后的状态表达，让阶段恢复更容易理解。
- 提升 inspect 和 stage.inspect_runs 的信息密度，方便快速定位异常。
...
1. `P0.1` 接入与运维文档补齐
2. `P0.2` 真实 provider 联调 smoke 流程
3. `P0.3` 运行入口与仓库形态统一
4. `P1.1` 多 LLM workflow 真并发
5. `P1.3` 历史版本与可追踪性增强
6. `P1.4` 可观测性与失败恢复增强
7. `P1.2` 术语模型扩展
8. `P2` 体验与产品化优化
